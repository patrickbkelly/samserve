...
origins = [
    ...
    "https://addingfuture.com",
    "https://addingfuture.com/contact/sample/industrial/",
    "https://addingfuture.com/contact/sample/adhesive-sealants/",
    "https://addingfuture.com/contact/sample/architectural/",
    "https://addingfuture.com/contact/sample/construction/",
    "https://localhost",
...
async def startup_event():
    ...
    logger.info("Allowed hosts: {}".format(ALLOWED_HOSTS))
...
def check_ip(ip):
    ...
    if ip not in ALLOWED_HOSTS:
        logger.error("IP {} not allowed".format(ip))
        return False
    return True

def check_token(x_token):
    global API_TOKENS
    if x_token:
        if x_token[0] in API_TOKENS:
            logger.info('Token passes')
            return True
        else:
            logger.error('Token ',str(x_token),' not allowed')
    return False

def check_request(request, ip):
    if check_ip(ip):
        result = {'result': 'success', 'reason': 'ip test success' }
    else:
        x_token = request.headers.get('x-token')
        if x_token:
            if check_token(x_token):
                result = {'result': 'success', 'reason': 'token test success' }
            else:
                result = {'result': 'fail', 'reason': ' token test failure' }
        else:
            result = {'result': 'fail', 'reason': 'missing x_token' }
    return result

@app.post("/api/testRequest")
async def testRequest( request: Request):
    logger.info('Client IP:', str(request.client.host))
    return check_request(request, '')

@app.post("/api/submitRequest")
async def submitRequest(sampleRequest: SampleRequest, request: Request):
